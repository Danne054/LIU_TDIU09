...
using Destination_Time_Type = vector<Time_Type>;
using Timetable_Type = vector<Destination_Time_Type>;


void get(Timetable_Type & timetable)
{
  Destination_Time_Type destination_timetable{};
  Time_Type time{};
  ...
  for (int i{}; i < 10; ++i)
	...
	{
	  get(time);
	  destination_timetable.push_back(time);
	}
      
      cin.ignore(1000, '\n');
      
      timetable.push_back(destination_timetable);
      destination_timetable.clear();
    }
}
